...
// Layout tokens (brand feel)
const PANEL_W = 560;
const PANEL_H = 360;
const MARGIN  = 24;
const GAP     = 20;   // space between rail icon and panel
const OFFSET_Y = -40; // nudge up to avoid HUD overlap
...
export default function MediaChooser() {
  ...
  React.useEffect(() => {
    const onOpen = (e: Event) => {
      ...
      // Anchor to the right-rail Media chip (aria-label="Media")
      ...
      if (btn) {
        ...
        const left = Math.max(MARGIN, Math.min(r.left - GAP - PANEL_W, window.innerWidth - PANEL_W - MARGIN));
        setPos({ top, left });
      } else {
        // Fallback: bottom-right, above HUD
        setPos({ top: Math.max(MARGIN, window.innerHeight - PANEL_H - 120), left: Math.max(MARGIN, window.innerWidth - PANEL_W - (MARGIN + 380)) });
      }
      setOpen(true);
    };
    ...
    >
      <header style={{padding: "14px 16px 8px", display: "flex", alignItems: "center"}}>
        ...
        <button
          ...
          aria-label="Close"
          style={{marginLeft:"auto", width:32, height:32, borderRadius:8, border:"1px solid rgba(148,163,184,.35)", background:"rgba(255,255,255,.8)", display:"grid", placeItems:"center"}}
        >×</button>
      </header>

      <div style={{padding: 16, paddingTop: 8, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, flex: 1}}>
        {items.map((it, i) => (
          <button
            key={i}
          ...
          >
            {it.type === "image" && (
              <img src={it.src} alt={it.label || "image"} style={{width:"100%", height:"100%", objectFit:"cover"}} />
            )}
            {it.type === "video" && (
              <div style={{width:"100%", height:"100%", display:"grid", placeItems:"center"}}>
                <div style={{width:62, height:62, borderRadius:"50%", background:"#1f2937", display:"grid", placeItems:"center"}}>
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="#fff" aria-hidden><path d="M8 5v14l11-7z"/></svg>
                </div>
              </div>
            )}
            {it.type === "audio" && (
              <div style={{width:"100%", height:"100%", display:"grid", placeItems:"center", color:"#0f172a"}}>Podcast</div>
            )}
            {it.label && (
              <span style={{
                position:"absolute", left:10, bottom:10,
                background:"rgba(236,252,203,.9)", color:"#052e16",
                border:"1px solid rgba(163,230,53,.6)", borderRadius:999, padding:"4px 10px",
