...
# --- Create IconsSprite component (so we don't paste by hand) ---
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center rounded-full p-2 
                  hover:bg-black/10 focus-visible:outline-none 
...
# --- Inject IconsSprite into app/layout.tsx if present ---
LAYOUT="$APP_DIR/layout.tsx"
if [ -f "$LAYOUT" ]; then
  cp "$LAYOUT" "$LAYOUT.bak.icons"
  # Insert import safely (respect 'use client')
  if grep -q "^['\"]use client['\"];$" "$LAYOUT"; then
    sed -i "0,/^['\"]use client['\"];$/ s//&\nimport IconsSprite from '.\/components\/IconsSprite';/" "$LAYOUT"
  else
    sed -i "1s/^/import IconsSprite from '.\/components\/IconsSprite';\n/" "$LAYOUT"
  fi
  # Insert <IconsSprite /> right after first <body ...>
  sed -i "0,/<body[^>]*>/ s//&\n        <IconsSprite \/>/" "$LAYOUT"
  echo "✅ Injected IconsSprite into $LAYOUT (backup: $LAYOUT.bak.icons)"
else
  echo "⚠️  App Router layout not found at $LAYOUT."
  echo "   If you're on Pages Router, add this to pages/_document.tsx inside <body>:"
  echo "     import IconsSprite from '../app/components/IconsSprite';"
  echo "     <IconsSprite />"
fi

echo "✅ Files created in $COMP_DIR and $PUB_DIR"
echo "Next: use them in your components, e.g.:"
echo "  import { Icon } from './components/Icon'"
echo "  import { IconButton } from './components/IconButton'"
echo "  <IconButton icon='info' label='Info' />"
