 ...
 * - No global `L`
 * - Dynamically imported inside useEffect (browser only)
 * - Safe to import without breaking SSR/prerender
 */
export default function PropertyMap({ lat, lng, zoom = 14, className = '' }: Props) {
  ...
  return <div ref={el} className={`w-full h-[240px] rounded-xl border border-neutral-200 ${className}`} aria-label="Map" />;
}
