...
export default function GhostSlider() {
  ...
  return (
    <div
      className="relative w-28 h-8 rounded-full bg-white/40 backdrop-blur-lg cursor-pointer transition-all border border-white/60"
      onClick={() => setMarket(isCommercial ? "domestic" : "commercial")}
      aria-label={`Switch to ${isCommercial ? "Domestic" : "Commercial"}`}
      role="switch"
