...
export default function ConnectMenu({ compact=false, className='', onPick }:{
  compact?: boolean; className?: string; onPick?: (s:PickType)=>void;
}) {
  ...
  const Item = ({label,icon,val}:{label:PickType;icon:React.ReactNode;val:PickType}) => (
    <button onClick={()=>{onPick?.(val);setOpen(false);}}
      className="w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-neutral-50 text-sm" role="menuitem">
      <span className="w-4 h-4 inline-flex">{icon}</span>{label}
    </button>
  );
  return (
    <div ref={ref} className={`relative ${className}`}>
      <button aria-label="Connect" onClick={()=>setOpen(v=>!v)}
        className={compact?"p-2 rounded-md bg-white/90 border border-neutral-200 shadow-sm hover:bg-white":
